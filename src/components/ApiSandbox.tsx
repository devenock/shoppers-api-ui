...
// const mockApiResponse = (endpoint: Endpoint): Promise<{ status: number; data: any }> => {
...
//                     const exampleData = successResponse.example.trim() ? JSON.parse(successResponse.example) : null;
//                     resolve({ status: successResponse.status, data: exampleData });
//                 } catch (e) {
//                     // If parsing fails, return the example as a string
//                     resolve({ status: successResponse.status, data: successResponse.example });
//                 }
//             } else {
...
// };
//
// const ApiSandbox: React.FC<ApiSandboxProps> = ({ apiVersion, apiEndpoints }) => {
//     const [selectedSection, setSelectedSection] = useState<string>(apiEndpoints[0]?.id || '');
...
//     const currentSection = apiEndpoints.find(section => section.id === selectedSection);
...
//
//         try {
//             const result = await mockApiResponse(currentEndpoint);
//             setResponse(result);
//         } catch (error) {
//             setResponse({
//                 status: 500,
...
//             <h2>API Sandbox</h2>
//             <p>Test the API endpoints directly in the browser</p>
//
...
//                                                 selectedEndpoint === index ? 'selected' : ''
//                                             }
...
//                                                     param.name,
//                                                     param.dataType === 'number' ? Number(e.target.value) : e.target.value
//                                                 )}
...
//                                                 Status: <span className={`status-${Math.floor(response.status / 100)}xx`}>{response.status}</span>
//                                             </div>
...
//                                         </div>
//                                     ) : (
//                                         <div className="no-response">Execute the request to see the response</div>
//                                     )}
//                                 </div>
//                             )}
//                         </>
