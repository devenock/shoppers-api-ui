...
async function makeRequest() {
  try {
    const response = await axios({
      ...
      url: '${formattedUrl}',${hasBody && endpoint.requestBodyExample ? `
...
func main() {${hasBody && endpoint.requestBodyExample ? `
    ...
    // Render code examples with tabs for different languages
    const CodeExamples: React.FC<{ endpoint: Endpoint, baseUrl: string }> = ({ endpoint, baseUrl }) => {
        const [activeTab, setActiveTab] = useState<'curl' | 'javascript' | 'nodejs' | 'golang'>('curl');
        const examples = generateCodeExample(endpoint, baseUrl);
        ...
        return (
            <div className="code-examples">
                ...
                <div className="code-content">
                    <div className="tooltip" style={{ position: 'absolute', top: '10px', right: '10px', zIndex: 10 }}>
                        {copySuccess && (
                    ...
                    <SyntaxHighlighter
                        language={activeTab === 'curl' ? 'bash' : activeTab === 'javascript' || activeTab === 'nodejs' ? 'javascript' : 'go'}
                        style={docco}
                                    ...
                                    <div className="endpoint-details">
                                            ...
                                            <>
                                                ...
                                                <div style={{ position: 'relative' }}>
                                                    <div className="tooltip" style={{ position: 'absolute', top: '10px', right: '10px', zIndex: 10 }}>
                                                        ...
                                                        <button
                                                            onClick={() => copyToClipboard(endpoint.requestBodyExample || '')}
                                                            style={{
                                                                background: 'none',
                                                                border: 'none',
                                                                color: '#6b7280',
                                        ...
                                        <table className="responses-table">
                                            ...
                                            {endpoint.responses.map((response, respIndex) => (
                                                <tr key={respIndex}>
                                                    <td>
                              <span className={`status-${Math.floor(response.status / 100)}xx`}>
                                ...
                                <span className={`status-${Math.floor(response.status / 100)}xx`}>
                                  ...
                                  Status: {response.status}
                                                                ...
                                                                <div className="tooltip" style={{ marginLeft: 'auto' }}>
                                                                    {copySuccess && (
                                                                        <span className="tooltip-text">{copySuccess}</span>
                                                                    )}
                                                                    <button
                                                            ...
                                                            {response.example ? (
                                                                <SyntaxHighlighter
                                                                    language="json"
                                                                    style={docco}
                                                                    customStyle={{ margin: 0, borderRadius: '0 0 var(--border-radius) var(--border-radius)' }}
                                                                >
                                                                    {response.example}
                                                                </SyntaxHighlighter>
                                                            ) : (
